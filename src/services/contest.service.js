...
function normalizeImages(payload) {
  const coverImage = payload.image?.trim();
  ...
  const cleanedImages = Array.from(
    new Set(
      [coverImage, ...images].map((item) => item?.trim()).filter(Boolean),
    ),
  );
...
function normalizeContestPayload(payload) {
  ...
  if (
    ...
    price < 0.5 ||
    !Number.isFinite(prizeMoney) ||
    prizeMoney < 0
  ) {
    const error = new Error(
      "Price must be at least 0.50 and prize money must be a valid positive number",
    );
    error.statusCode = 400;
    throw error;
  }
...
function getContestSort(sort) {
  const sortOptions = {
    deadline: { deadline: 1, createdAt: -1 },
    prize: { prizeMoney: -1, createdAt: -1 },
    entry: { price: 1, createdAt: -1 },
    popular: { participantCount: -1, createdAt: -1 },
    newest: { createdAt: -1 },
  };
...
function applyExtraContestFilters(filter, { maxPrice, deadline } = {}) {
...
export async function listApprovedContests({
  type,
  search,
  page = 1,
  limit = 9,
  ...
  const filter = applyExtraContestFilters(
    approvedContestFilter({ type, search }),
    { maxPrice, deadline },
  );
...
export async function listRelatedContests(id, type, limit = 4) {
  if (!type) return [];
  ...
  const contests = await contestsCollection()
    ...
    .sort({ participantCount: -1, createdAt: -1 })
    .limit(Math.min(Math.max(Number(limit) || 4, 1), 8))
    .toArray();
...
export async function updateCreatorContest(id, email, payload) {
  ...
  if (contest.status === "approved") {
    ...
    const hasOnlyAllowedFields =
      ...
      payloadFields.every((field) => allowedFields.includes(field));

    if (!hasOnlyAllowedFields) {
      const error = new Error(
        "Only deadline, description, and task instruction can be changed after approval",
      );
    ...
    if (contest.winnerName || contest.winnerUserId) {
      const error = new Error(
        "Deadline cannot be changed after a winner is declared",
      );
      error.statusCode = 409;
      throw error;
    }
    ...
    if (Object.prototype.hasOwnProperty.call(payload, "images")) {
      updates.images = normalizeImages({ ...contest, images: payload.images });
    }
...
export async function listAllContestsForAdmin({
  ...
  const filter = ["pending", "approved", "rejected"].includes(normalizedStatus)
    ...
    : {};
  const sort = getContestSort(sortKey);
...
export async function updateContestStatus(id, status) {
  ...
  if (
    status === "approved" &&
    contest.deadline &&
    new Date(contest.deadline).getTime() <= Date.now()
  ) {
